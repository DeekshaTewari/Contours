...
Args = struct('salience',1,'CR',1,'SacAlign',0,'nHist',0,'nlimit',6,'Flimit',150,'bins',20,'binsEdges',[],'maxcolorscale',1);
...
if length(limit) == 1 & ~Args.nHist
    ...
    if nused >= Args.nlimit
        ...
        imagesc(P(:,1:nanlim),[-1 Args.maxcolorscale]);
        hax = gca;
        ...
        plot(repmat(xevent,1,length(yrange)),yrange,'b');
