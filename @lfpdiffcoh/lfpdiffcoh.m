function obj = lfpdiffcoh(varargin)
% @lfpdiffcohl Constructor function for lfpdiffcoh class
%
%
%   Dependencies:

Args = struct('RedoLevels',0,'SaveLevels',0,'Auto',0,'corTrial',[1 2],'IncorTrial',[-1 -2 -3 -4]);
...
Args.classname = 'lfpdiffcoh';
Args.matname = [Args.TrialType{:} 'coh.mat'];
Args.matvarname = sprintf('lfp%s%scoh',Args.TrialType{:});
...
if ~isempty(lfpt)
    ...
    if lfpt.data.Nchannels > 1
        comb = nchoosek([1:lfpt.data.Nchannels],2);
        ch = size(comb,1);
        for sal = 1 : Args.totsal
            fprintf('\n salience %d ',sal)

            [Dif,para,status(sal)] = compareLFP(lfpt,Args.trialtype,sal,'cohgram',varargin{:});
            if status(sal)
        ...
        if ~isempty(find(status == 1))
            ...
            fprintf('None of the salience condition satisfies the requirement \n');
            obj = createEmptyObject(Args);
        end
    else
        fprintf('Only one channel, no cohgram possible \n');
        obj = createEmptyObject(Args);
    end
else
